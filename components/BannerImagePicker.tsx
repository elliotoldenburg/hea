...
const CACHE_TIME = 31536000; // 1 year in seconds

export default function BannerImagePicker({ currentImageUrl, onImageUpdate }: Props) {
  ...
  const fetchBannerImage = useCallback(async () => {
    try {
      ...
      if (data?.banner_image_url) {
        const url = new URL(data.banner_image_url);
  ...
  const requestPermissions = async (useCamera: boolean) => {
    ...
    try {
      ...
      if (!permissionResult.granted) {
        Alert.alert(
          'Behörigheter krävs',
          'För att kunna välja eller ta en bannerbild behöver du ge tillgång till ' + 
  ...
  const uploadImage = async (uri: string) => {
    try {
      ...
      console.log('Uploading new banner image:', filePath);
      ...
      console.log('Banner image uploaded successfully. URL:', newImageUrl);

      const { error: updateError } = await supabase
        .from('training_profiles')
        .update({ banner_image_url: newImageUrl })
        .eq('user_id', user.id);

      if (updateError) throw updateError;

      console.log('Profile updated with new banner image URL');
  ...
  const showImageOptions = () => {
    Alert.alert(
      'Välj bannerbild',
      'Hur vill du välja din bannerbild?',
      [
        {
          text: 'Ta en ny bild',
    ...
    <View style={styles.container}>
      ...
      >
        {uploading ? (
          ...
          <>
            <ImageIcon size={48} color="#808080" />
