/*
  ...
  1. New Tables
    - `meals` for tracking user meals
    - `meal_items` for tracking individual food items in meals
...
BEGIN
  ...
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'meals') THEN
    CREATE TABLE meals (
  ...
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'meal_items') THEN
...
DROP TRIGGER IF EXISTS update_meal_totals_insert_update ON meal_items;
DROP TRIGGER IF EXISTS update_meal_totals_delete ON meal_items;
DROP FUNCTION IF EXISTS update_meal_totals CASCADE;
DROP FUNCTION IF EXISTS get_daily_nutrition_summary_for_meals CASCADE;
DROP FUNCTION IF EXISTS get_meal_with_items CASCADE;
...
CREATE OR REPLACE FUNCTION get_daily_nutrition_summary_for_meals(p_date date DEFAULT CURRENT_DATE)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION get_daily_nutrition_summary_for_meals TO authenticated;
