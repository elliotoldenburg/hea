...
BEGIN
  RETURN QUERY
  SELECT 
    COALESCE(SUM(dml.total_calories)::integer, 0) as total_calories,
    COALESCE(SUM(dml.total_protein)::integer, 0) as total_protein,
    COALESCE(SUM(dml.total_carbs)::integer, 0) as total_carbs,
    COALESCE(SUM(dml.total_fat)::integer, 0) as total_fat
  FROM daily_meal_logs dml
  WHERE 
    dml.user_id = p_user_id AND
    dml.log_date = p_date;
