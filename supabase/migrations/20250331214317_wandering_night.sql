...
DROP FUNCTION IF EXISTS handle_friend_request CASCADE;
DROP FUNCTION IF EXISTS get_friends_with_profiles CASCADE;
DROP FUNCTION IF EXISTS get_friend_progress CASCADE;
...
-- Update friends table structure if it exists
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE friends ADD COLUMN updated_at timestamptz DEFAULT now();
  END IF;
