...
-- Create a simple, effective search function
...
BEGIN
  ...
  IF search_type = 'username' THEN
    ...
    WHERE 
      ...
      LOWER(tp.username) LIKE LOWER(search_query || '%') AND
      (current_user_id IS NULL OR tp.user_id != current_user_id)
  ...
  ELSE
    ...
    FROM training_profiles tp
    WHERE 
      LOWER(tp.full_name) LIKE LOWER('%' || search_query || '%') AND
      (current_user_id IS NULL OR tp.user_id != current_user_id)
