...
CREATE OR REPLACE FUNCTION recalc_daglig_totals()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_insert ON maltidsinlagg;
DROP TRIGGER IF EXISTS trg_update ON maltidsinlagg;
DROP TRIGGER IF EXISTS trg_delete ON maltidsinlagg;

CREATE TRIGGER trg_insert
  AFTER INSERT ON maltidsinlagg
  FOR EACH ROW EXECUTE FUNCTION recalc_daglig_totals();

CREATE TRIGGER trg_update
  AFTER UPDATE ON maltidsinlagg
  FOR EACH ROW EXECUTE FUNCTION recalc_daglig_totals();

CREATE TRIGGER trg_delete
  AFTER DELETE ON maltidsinlagg
  FOR EACH ROW EXECUTE FUNCTION recalc_daglig_totals();
